...
def download_invoice_view(request,orderID,productID):
    ...
    template = render_to_string('ecom/download_invoice.html', mydict)
...
def generate_pdf(template):
    pdf_data = BytesIO()
    pisa.CreatePDF(template, dest=pdf_data)
...
def contactus_view(request):
    ...
    return render(request, 'ecom/contact.html', {'form':sub})


def testimonial_view(request):
    return render(request,'ecom/testimonial.html')
...
def customer_contact_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email),message, settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVING_USER, fail_silently = False)
            return render(request, 'ecom/contactussuccess.html')
    return render(request, 'ecom/customer-contact.html', {'form':sub})
